package ASparkStarter::App::IO::Out::TxtWriter;
...
=head1 SYNOPSIS
	...
	doResolves the product version and base dirs , bootstraps cnfig files if needed
		...
		use ASparkStarter::App::ETL::ASparkStarter ;
		my $objASparkStarter = 
			'ASparkStarter::App::ETL::ASparkStarter'->new ( \$appConfig ) ; 
=cut 
...
sub doConvertHashRefToStr {
  ...
  my $objTimer         = 'ASparkStarter::App::Utils::Timer'->new( $appConfig->{ 'TimeFormat' } );
...
sub doInitialize {
  ...
  $objFileHandler
    = 'ASparkStarter::App::Utils::IO::FileHandler'->new(\$appConfig);
  ...
  %inverse_hsrStatus = reverse %$hsrStatus;
   ...
   my $aspark_starter_project = $appConfig->{ 'aspark_starter_project' } ; 
    ...
    $issues_file
      ...
      . "/$year/$nice_month/$nice_date/$aspark_starter_project"
      . '.'
      . "$table"
      . '.'
      . "$nice_date"
...
sub get {
  ...
  my $self = shift;
  my $name = shift;
  croak "\@ASparkStarter.pm sub get TRYING to get undefined name" unless $name;
  croak "\@ASparkStarter.pm sub get TRYING to get undefined value"
    unless ($self->{"$name"});
