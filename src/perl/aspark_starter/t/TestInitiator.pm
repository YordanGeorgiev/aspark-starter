...
my $objInitiator 				= 'ASparkStarter::App::Utils::Initiator'->new();	
my $appConfig					= {} ;
...
ok ( $ProductDir 				eq '/opt/csitea/aspark-starter' ) ; 
ok ( $ProductInstanceDir 	eq '/opt/csitea/aspark-starter/aspark-starter.0.1.3.dev.ysg' ); 
ok ( $ProductVersion 		eq '0.1.3' ); 
ok ( $EnvironmentName 		eq 'aspark-starter.0.1.3.dev.ysg' ); 
...
ok ( $ConfFile					eq 
'/opt/csitea/aspark-starter/aspark-starter.0.1.3.dev.ysg/cnf/aspark-starter.' . $ProductType . '.' . $cmd_out . '.cnf' ) ; 
