...
# bash src/bash/aspark-starter/aspark-starter.sh -a to-app=<<new_app>>
#------------------------------------------------------------------------------
doCloneToApp(){
	tgt_app="$1"
	prefix='to-app='
	tgt_app=${tgt_app#$prefix}
	tgt_product_instance_env_name=$(echo $product_instance_env_name | perl -ne "s/$run_unit/$tgt_app/g;print")
