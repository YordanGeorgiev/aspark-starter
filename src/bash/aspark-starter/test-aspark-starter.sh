#!/bin/bash 
# file: src/bash/aspark-starter/test-aspark-starter.sh
...
doRunTests(){
   ...
   source lib/bash/funcs/log-test-run-entry.func.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-aspark-starter loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/aspark-starter/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/aspark-starter/tests/run-aspark-starter-tests.lst)
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub aspark-starterer
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/aspark-starter/test-aspark-starter.sh
