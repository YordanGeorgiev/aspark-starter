...
doTestCreateRelativePackage(){
   ...
   exit_code=0
	bash src/bash/aspark-starter/aspark-starter.sh -a create-relative-package
   export exit_code=$?
   ...
   bash src/bash/aspark-starter/aspark-starter.sh -a create-relative-package -i met/.tst.aspark-starter
   export exit_code=$?
   ...
   test $exit_code -ne 0 && return
	...
	bash src/bash/aspark-starter/aspark-starter.sh -a create-relative-package -i met/.prd.aspark-starter
   export exit_code=$?
   ...
   # test the call with the include file of the prd env
   bash src/bash/aspark-starter/aspark-starter.sh -a create-relative-package -i met/.git.aspark-starter
   export exit_code=$?
